<template>
    <div class="card" v-if="meal">
      ...
      <div style="display:inline-block">
          <span v-if="!editable" @click="editable=true" >{{meal.disc}}</span>
          <div v-else>
              <input v-model="newText" />
              <button @click="hide"> done editing </button>
          </div>
      </div><br>
      <b>Calories:</b> {{meal.calories}} <br><br>
      <img :src="meal.image" style = "width: 8em; height: 7em" onclick = "this.play();" v-if="meal.image">
      <br>
...
export default {
    ...
    firestore: function() {
        return {
          meal: db.collection("breakfast").doc(this.id)
        }
    },
    methods: {
        doDelete: function() {
            this.$firestoreRefs.meal.delete()
        },
        hide: function() {
            this.editable = false;
            this.meal.disc = this.newText;
            this.$emit("changed", this.text);
            this.$firestoreRefs.meal.set(this.meal)
        }
    }
}
</script>
